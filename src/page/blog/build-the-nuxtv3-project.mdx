...
文档：[https://image.nuxt.com/get-started/installation](https://image.nuxt.com/get-started/installation)
...
export const useLangStore = defineStore("languageStore", {
  ...
  getters: {
    getLanguage: (state) => {
      if (process.client) {
        return (
          (state.language as ILanguageState) ||
          (localStorage.getItem("language") as ILanguageState)
        );
      }
    },
  },
